...
pub fn get_latest_log_path(dir: &Path) -> PathBuf {
    let mut latest_path = dir.join("arti.log");
...
pub fn tail_file(log_dir: PathBuf, tx: tokio::sync::mpsc::Sender<AppEvent>) {
    tokio::spawn(async move {
        // Find latest log file initially
        let mut path = get_latest_log_path(&log_dir);
