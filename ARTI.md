...
This document provides a summary of Arti, the Rust-based Tor implementation, as it is configured and built within this project.

---

## What It Is

Arti is the next-generation official implementation of the Tor protocol, written in the Rust programming language. It is designed from the ground up to be a safer, more modular, and easier-to-maintain alternative to the original C implementation of Tor. Its goals are to improve security, performance, and correctness through the safety guarantees provided by Rust.

## How It Was Obtained and Built

- **Source:** Arti was cloned as a Git submodule into this project from the official Tor Project repository: `https://gitlab.torproject.org/tpo/core/arti.git`.
- **Location:** The source code for the submodule is located at `umbra/arti/`.
- **Build Process:** It was compiled using Cargo, the Rust build tool, with the command `cargo build --release`. This command creates a binary that is optimized for the host machine's architecture (in this case, Apple Silicon).
- **Executable Location:** The compiled executable can be found at `umbra/arti/target/release/arti`.

## How to Use It

To run Arti as a SOCKS proxy, execute the following command from the `umbra/arti` directory:

```bash
./target/release/arti proxy
```

By default, this will start a SOCKS proxy listening on **port 9150** on your local machine. You can then configure your applications (like a web browser) to use `socks5://127.0.0.1:9150` to route their traffic through the Tor network.

## How to Upgrade It

Since Arti is included as a Git submodule, upgrading it is a two-step process:

1.  **Update the Submodule:** First, navigate into the submodule's directory and pull the latest changes from its repository:
    ```bash
    cd umbra/arti
    git pull
    cd ../.. 
    ```
2.  **Commit the New Version:** After updating the submodule, the parent `umbra` repository will see this change. You must commit this update to lock the parent repository to the new version of Arti:
    ```bash
    git add arti
    git commit -m "Update arti submodule to latest version"
    git push
    ```
After upgrading, you will need to rebuild the project with `cargo build --release` inside the `umbra/arti` directory.

## How to Administer It (Configuration)

Arti is configured using a TOML file, typically named `arti.toml`. For this project, the canonical configuration file is located at `umbra/arti.toml` and is under version control.

This file is symbolically linked to the default location that the `arti` executable expects on macOS. This allows for easy editing and tracking of configuration changes within the project.

- **Project Location:** `umbra/arti.toml`
- **Symlinked To:** `~/Library/Application Support/org.torproject.Arti/arti.toml`
-   **Custom Location:** You can still specify a different configuration file using the `-c` flag:
    ```bash
    ./target/release/arti -c /path/to/your/arti.toml proxy
    ```

### Example Configuration

A fully-documented example configuration file is included in the Arti source code at `umbra/arti/crates/arti/src/arti-example-config.toml`. You can use this as a template for your own `arti.toml`.

Below is the content of that example file for your reference:
